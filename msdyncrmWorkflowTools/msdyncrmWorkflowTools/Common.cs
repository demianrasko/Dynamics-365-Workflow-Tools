...
namespace msdyncrmWorkflowTools
{
    class Common
    {
        ...
        public Guid CloneRecord(string entityName, string objectId)
        {
            Entity retrievedObject = this.service.Retrieve(entityName, new Guid(objectId), new ColumnSet(allColumns: true));
            this.tracingService.Trace("retrieved object OK");

            Entity newEntity = new Entity(entityName);
            string PrimaryIdAttribute = "";
            List<string> atts = this.getEntityAttributesToClone(entityName, this.service, ref PrimaryIdAttribute);
            ...
            foreach (string att in atts)
            {
                    ...
                    || att.StartsWith("partylist-"))
                {
                    if (att.StartsWith("partylist-"))
                    {
                        ...
                        this.tracingService.Trace(fetchParty);
                        EntityCollection returnCollection = ((RetrieveMultipleResponse)this.service.Execute(fetchRequest1)).EntityCollection;


                        EntityCollection arrPartiesNew = new EntityCollection();
                        this.tracingService.Trace("attribute:{0}", att2);
                        ...
                        foreach (Entity ent in returnCollection.Entities)
                        {
                            ...
                            this.tracingService.Trace("attribute:{0}:{1}:{2}", att2, partyid.LogicalName, partyid.Id.ToString());
                    ...
                    this.tracingService.Trace("attribute:{0}", att);
                    newEntity.Attributes.Add(att, retrievedObject.Attributes[att]);
                }
            }

            this.tracingService.Trace("creting cloned object...");
            Guid createdGUID = this.service.Create(newEntity);
            this.tracingService.Trace("created cloned object OK");

            Entity record = this.service.Retrieve(entityName, createdGUID, new ColumnSet("statuscode", "statecode"));
                ...
                retrievedObject.Attributes["statecode"] != record.Attributes["statecode"])
            {
                ...
                this.service.Update(setStatusEnt);
            }

            this.tracingService.Trace("cloned object OK");
            return createdGUID;
        }
